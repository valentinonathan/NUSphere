...
export default async function ModulesPage() {
    ...
    const feedModules = await fetchBackendServer<module[]>("/modules/feed", "GET");
    return (
        <div className="p-4">
            ...
            <div className="flex flex-col gap-8">
                <div className="flex flex-col gap-4">
                    ...
                    <div className="flex gap-4 flex-wrap">
                        {modules?.map(m => <ModuleBox key={m?.id} banner={m?.banner_url} moduleCode={m?.title} />)}
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="flex gap-4 flex-wrap">
                        {feedModules?.map(m => <ModuleBox key={m?.id} banner={m?.banner_url} moduleCode={m?.title} />)}
                    </div>
                </div>
            </div>
        </div>
