        ...
        return (
            <div className="flex flex-col gap-2 min-w-full max-w-full">
                ...
                {usersResult?.length == undefined || usersResult?.length == 0 
                        ? null 
                        : usersResult.map(u => <PeopleBadge key={u?.id} imageUrl={u?.pfp_url} username={u?.username} first_name={u?.first_name} last_name={u?.last_name} year={u?.year} residence={u?.residence} nationality={u?.nationality} major={u?.major} />)
                }
                </div>
            </div>
        );
